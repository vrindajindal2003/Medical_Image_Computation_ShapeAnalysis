function mean_pt_set = mean_finder(pt_set)
    no_of_shapes = 150;
    r = randi([1 150],1,1);          %generate a random number from 1-40.
  ...
  % while(diff > 0.1) 
  for j=1:5                 %Currently working with 5 iterations of algorithm
    ...
    x_aligned = reshape(x_aligned,16,150);
    
    y_aligned = aligned_pt_set1(2,:,:);
    y_aligned = reshape(y_aligned,16,150);
    ...
    z_mean = align_pointset(last_mean , mean_pt_set);   %to check how much mean converges to last mean
