function mean_pt_set = mean_brain_finder(pt_set)
    no_of_shapes = 40;
  ...
  for j=1:15                 %Currently working with 5 iterations of algorithm
    ...
    x_aligned = reshape(x_aligned,32,40);
    
    y_aligned = aligned_pt_set1(2,:,:);
    y_aligned = reshape(y_aligned,32,40);
    ...
    z_mean = align_brain_pointset(last_mean , mean_pt_set);   %to check how much mean converges to last mean
